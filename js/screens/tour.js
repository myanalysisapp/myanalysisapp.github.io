...
export function openTour(ctx) {
  ...
  function goStep(n) {
    ...
    const s = STEPS[step];
    /* Переключаем раздел, только если он и правда другой. Раньше вызов
       шёл на каждом шаге, и пять шагов подряд по «Сегодня» перерисовывали
       экран заново: кольцо каждый раз рисовалось с нуля, счётчики бежали
       от нуля, прокрутка возвращалась наверх. Со стороны это выглядело
       как прыжок на начальный экран. */
    if (s.go && ctx && ctx.go && location.hash !== '#/' + s.go) ctx.go(s.go);
    draw();
  }
